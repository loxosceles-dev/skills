...
- **Thumbprint**: `1c58a3a8518e8759bf075b76b750d4f2df264fcd` — **vestigial.** Since July 6, 2023 AWS validates GitHub's OIDC server certs against its trusted-CA library and ignores the configured thumbprint for `token.actions.githubusercontent.com`. Any 40-hex value works; the API just requires the field. Do not chase thumbprint errors — they are a 2022 problem.
...
**Environment-scoped trust (when the workflow sets `environment:`):**

If a job uses GitHub **Environments** (`environment: <name>` in the job — needed for environment-scoped secrets/vars), the OIDC `sub` references the **environment name, not the branch ref**:

```
# Job with environment: dev → sub is:
"repo:owner/repo:environment:dev"
"repo:owner@{id}/repo@{id}:environment:dev"   # immutable format
```

So the trust policy must pin `:environment:{name}`, and there is NO branch info in the claim:

```json
"token.actions.githubusercontent.com:sub": "repo:owner/repo:environment:prod"
```

**Tradeoff — branch restriction moves out of the trust policy.** Because the `sub` only carries the environment name, the role is assumable by *any* run that resolves to that environment, regardless of branch. Branch scoping must then be enforced elsewhere:
- The workflow's own guard (e.g. `if: github.ref == 'refs/heads/dev' || ...`) — only prevent the job from running; it does not stop a modified workflow.
- GitHub Environment **protection rules** — restrict the environment (and its secrets) to specific branches (`main` for prod). This is the real lock: an environment restricted to `main` will not issue a token/run with `environment:prod` from other branches. Set this if the environment gates prod access.

If you need branch identity *inside* the claim itself, do not use `environment:` for that job — use `ref:refs/heads/{branch}` scoping instead (and store secrets at repo level, not environment level).
...
**Cause**: Role missing OR trust policy conditions don't match the actual token `sub`/`aud`.

**This is the #1 recurring failure. Three distinct causes, same error — check in order:**

1. **Role doesn't exist** — setup script never ran (or ran without valid AWS creds). Check:
   ```bash
   aws iam get-role --role-name <role-name> --query 'Role.Arn' --output text
   # NoSuchEntity → run the setup script (with valid creds), then re-check
   ```

2. **Role exists but trust policy is STALE** — the role was created by an earlier version of the
   setup script and carries a wrong `sub` claim (name-based `repo:owner/repo:...` instead of
   immutable `repo:owner@{id}/repo@{id}:...`, or branch-scoped `:ref:refs/heads/...` instead of
   `:environment:...`). This silently persists because an "already exists → skip" check never
   repairs the policy. **Fix: re-apply the trust policy from the current script:**
   ```bash
   aws iam update-assume-role-policy \
     --role-name <role-name> \
     --policy-document "$(cat trust-policy.json)"
   ```
   **Setup scripts must ALWAYS re-apply the trust policy on every run (idempotent), never
   skip-if-exists** — a "role exists" early-return is a bug, not an optimization.

3. **Trust policy doesn't match workflow context** — repo name, sub format (immutable IDs!),
   environment name, or audience wrong.

**Check**:
1. Role exists (command above)
2. Role's CURRENT trust policy matches the repo's sub prefix:
   ```bash
   aws iam get-role --role-name <role-name> \
     --query 'Role.AssumeRolePolicyDocument.Statement[0].Condition' --output json
   # Compare the sub against:
   gh api "repos/{owner}/{repo}/actions/oidc/customization/sub" --jq '.sub_claim_prefix'
   # expected sub = <prefix>:environment:<env>   (when the job sets environment:)
   #              = <prefix>:ref:refs/heads/<branch>  (otherwise)
   ```
3. Workflow has `id-token: write` permission

**Debug**: print the actual token claims from inside the run:
```yaml
- name: Debug OIDC token
  run: |
    curl -s -H "Authorization: bearer $ACTIONS_ID_TOKEN_REQUEST_TOKEN" \
      "$ACTIONS_ID_TOKEN_REQUEST_URL&audience=sts.amazonaws.com" | jq -r '.sub'
```
The printed `sub` is the ground truth — the trust policy must match it exactly.
